...
def conv2d_ABN(ni, nf, stride, activation="leaky_relu", kernel_size=3,
               activation_param=1e-2, groups=1):
    return nn.Sequential(
...
class BasicBlock(Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, use_se=True,
        ...
        if stride == 1:
            self.conv1 = conv2d_ABN(inplanes, planes, stride=1,
                                    activation_param=1e-3)
        else:
            if anti_alias_layer is None:
                self.conv1 = conv2d_ABN(inplanes, planes, stride=2,
                                        activation_param=1e-3)
            else:
                self.conv1 = nn.Sequential(
                    conv2d_ABN(inplanes, planes, stride=1,
                               activation_param=1e-3),
...
class Bottleneck(Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, use_se=True,
        ...
        self.conv1 = conv2d_ABN(inplanes, planes, kernel_size=1, stride=1,
                                activation="leaky_relu",
                                activation_param=1e-3)
        if stride == 1:
            self.conv2 = conv2d_ABN(planes, planes, kernel_size=3, stride=1,
                                    activation="leaky_relu",
                                    activation_param=1e-3)
        else:
            if anti_alias_layer is None:
                self.conv2 = conv2d_ABN(planes, planes, kernel_size=3, stride=2,
                                        activation="leaky_relu",
                                        activation_param=1e-3)
            else:
                self.conv2 = nn.Sequential(
                    conv2d_ABN(planes, planes, kernel_size=3, stride=1,
                               activation="leaky_relu", activation_param=1e-3),
                    anti_alias_layer(channels=planes, filt_size=3, stride=2))

        self.conv3 = conv2d_ABN(planes, planes * self.expansion, kernel_size=1,
                                stride=1,
                                activation="identity")
...
class TResNet(Module):
    ...
    def __init__(self, layers, in_chans=3, num_classes=1000, width_factor=1.0,
        ...
        layer1 = self._make_layer(BasicBlock, self.planes, layers[0], stride=1,
                                  use_se=True,
    ...
    def _make_layer(self, block, planes, blocks, stride=1, use_se=True,
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            layers += [conv2d_ABN(self.inplanes, planes * block.expansion,
        ...
        layers = []
        layers.append(
            block(self.inplanes, planes, stride, downsample, use_se=use_se,
                  anti_alias_layer=anti_alias_layer))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks): layers.append(
            block(self.inplanes, planes, use_se=use_se,
                  anti_alias_layer=anti_alias_layer))
...
def TResnetL(model_params):
    ...
    model = TResNet(
        layers=[4, 5, 18, 3], num_classes=num_classes,
        in_chans=in_chans, width_factor=1.2,
...
def TResnetXL(model_params):
    ...
    num_classes = model_params['num_classes']
    model = TResNet(
        layers=[4, 5, 24, 3], num_classes=num_classes,
